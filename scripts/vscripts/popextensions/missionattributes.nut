...
MissionAttributes.Attrs <- {
	...
	function BotsAreHumans( value ) {
		...
		PopEventHook("post_inventory_application", "BotsAreHumans", function( params ) {
			...
			printl(player.IsEFlagSet( EFL_CUSTOM_WEARABLE ))
			...
			if ( player.GetTeam() == TF_TEAM_PVE_INVADERS && value > 0 ) {
				...
				if ( value & 2 ) {
					...
					PopExtUtil.GiveWearableItem( player, 9910, format( "models/player/items/%s/%s_zombie.mdl", classname, classname ) )
	...
	function ReverseMVM( value ) {
		...
		PopEventHook("post_inventory_application", "ReverseMVMSpawn", function( params ) {
			...
			PopExtUtil.RemoveThink( player, "ReverseMVMSpawn" )
