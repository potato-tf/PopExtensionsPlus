::CustomAttributes <- {
    ...
    Attrs = {
		function FiresMilkBolt(player, item, value) {
			...
			playerScope.PlayerThinkTable[format("FiresMilkBolt_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

				if (item == null || player.GetActiveWeapon() != item) return
			...
			CustomAttributes.TakeDamageTable[format("FireMilkBolt_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function AddCondOnHit(player, item, value) {
			...
			CustomAttributes.TakeDamageTable[format("AddCondOnHit_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function RemoveCondOnHit(player, item, value) {
			...
			CustomAttributes.TakeDamagePostTable[format("RemoveCondOnHit_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

				local victim = GetPlayerFromUserID(params.userid)
				local attacker = GetPlayerFromUserID(params.attacker)

				if (victim == null || attacker == null || !victim.IsPlayer() || !victim.InCond(value) || victim.IsInvulnerable() || attacker != player || params.weapon != item) return
		...
		function SelfAddCondOnHit(player, item, value) {
			...
			CustomAttributes.TakeDamageTable[format("SelfAddCondOnHit_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function SelfAddCondOnKill(player, item, value) {
			...
			CustomAttributes.DeathHookTable[format("SelfAddCondOnKill_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function FireInputOnHit(player, item, value) {
			...
			CustomAttributes.TakeDamageTable[format("FireInputOnHit_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

				if (params.attacker != player || params.weapon != item) return
		...
		function FireInputOnKill(player, item, value) {
			...
			CustomAttributes.DeathHookTable[format("FireInputOnKill_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

				if (GetPlayerFromUserID(params.attacker) != player || params.weapon != item) return
		...
		function MultDmgVsSameClass(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("DmgVsSameClass_%d_%d", player.GetScriptScope().userid, item.entindex())] <-  function(params) {
		...
		function MultDmgVsAirborne(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("MultDmgVsAirborne_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function TeleportInsteadOfDie(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("TeleportInsteadOfDie_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function MeleeCleaveAttack(player, item, value = 64) {
            ...
            scope.ItemThinkTable[format("MeleeCleaveAttack_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (scope.cleavenextattack == GetPropFloat(item, "m_flNextPrimaryAttack") || GetPropFloat(item, "m_fFireDuration") == 0.0 || player.GetActiveWeapon() != item || !("melee cleave attack" in player.GetScriptScope().attribinfo)) return

                scope.cleaved = false

                scope.cleavenextattack = GetPropFloat(item, "m_flNextPrimaryAttack")
            }
            CustomAttributes.TakeDamageTable[format("MeleeCleaveAttack_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                if (scope.cleaved || params.weapon != item || !("melee cleave attack" in player.GetScriptScope().attribinfo)) return
		...
		function TeleporterRechargeTime(player, item, value = 1.0) {
			...
			scope.ItemThinkTable[format("TeleporterRechargeTime_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function UberOnDamageTaken(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("UberOnDamageTaken_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function SetTurnToIce(player, item, value = null) {
            ...
            CustomAttributes.TakeDamageTable[format("TurnToIce_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function ModTeleporterSpeedBoost(player, item, value) {
            ...
            local scope = item.GetScriptScope()
            CustomAttributes.PlayerTeleportTable[format("ModTeleporterSpeedBoost_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function CanBreatheUnderWater(player, item, value = null) {
			...
			local painfinished = GetPropInt(player, "m_PainFinished")
				...
				local scope = item.GetScriptScope()
				scope.ItemThinkTable[format("CanBreatheUnderwater_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

					if (!("can breathe under water" in player.GetScriptScope().attribinfo) || player.GetActiveWeapon() != item) return
		...
		function MultSwimSpeed(player, item, value = 1.25) {
            ...
            local scope = item.GetScriptScope()
            scope.ItemThinkTable[format("MultSwimSpeed_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (!("mult swim speed" in player.GetScriptScope().attribinfo) || player.GetActiveWeapon() != item) return
		...
		function LastShotCrits(player, item, value) {
            ...
            scope.ItemThinkTable[format("LastShotCrits_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (!item || !("last shot crits" in player.GetScriptScope().attribinfo) || player.GetActiveWeapon() != item) return
		...
		function CritWhenHealthBelow(player, item, value = -1) {
            ...
            item.GetScriptScope().ItemThinkTable[format("CritWhenHealthBelow_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function WetImmunity(player, item, value = null) {
            ...
            item.GetScriptScope().ItemThinkTable[format("WetImmunity_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (player.GetActiveWeapon() != item) return
		...
		function BuildSmallSentries(player, item, value = null) {
            ...
            CustomAttributes.BuiltObjectTable[format("BuildSmallSentries_%d_%d", scope.userid, item.entindex())] <- function(params) {
            ...
            CustomAttributes.UpgradedObjectTable[format("BuildSmallSentries_%d_%d", scope.userid, item.entindex())] <- function(params) {
            ...
            CustomAttributes.QuickSentryUpgradeTable[format("BuildSmallSentries_%d_%d", scope.userid, item.entindex())] <- function(params) {
		...
		function RadiusSleeper(player, item, value = null) {
            ...
            CustomAttributes.TakeDamagePostTable[format("RadiusSleeper_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function ExplosiveBullets(player, item, value) {
            ...
            if (item.Clip1() != -1) scope.curclip <- item.Clip1()

            scope.ItemThinkTable[format("ExplosiveBullets_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (!("explosive bullets" in player.GetScriptScope().attribinfo) || player.GetActiveWeapon() != item || scope.explosivebulletsnextattack == GetPropFloat(item, "m_flLastFireTime")) return
                ...
                scope.curammo = GetPropIntArray(player, "m_iAmmo", item.GetSlot() + 1)
                if ("curclip" in scope) scope.curclip = item.Clip1()
		...
		function ExplosiveBulletsExt(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("ExplosiveBulletsExt_%d_%d", scope.userid, item.entindex())] <- function(params) {

                if ("explosivebullets" in scope || params.weapon != item || !("explosive bullets ext" in player.GetScriptScope().attribinfo)) return
		...
		function OldSandmanStun(player, item, value = null) {
            ...
            CustomAttributes.TakeDamageTable[format("OldSandmanStun_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
                local attacker = params.attacker
                local victim = params.const_entity
		...
		function StunOnHit(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("StunOnHit_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                if (!params.const_entity.IsPlayer() || params.weapon != item || (!stungiants && params.const_entity.IsMiniBoss())) return
		...
		function IsMiniboss(player, item, value = null) {
            ...
            item.GetScriptScope().ItemThinkTable[format("IsMiniBoss_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function ReplaceWeaponFireSound(player, item, value) {
            ...
            {
                scope.ItemThinkTable[format("ReplaceFireSound_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
            ...
            CustomAttributes.TakeDamageTable[format("ReplaceFireSound_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                local _weapon = params.weapon

                if (!_weapon || _weapon != item) return
		...
		function IsInvisible(player, item, value = null) {
            ...
            item.GetScriptScope().ItemThinkTable[format("IsInvisible_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (player.GetActiveWeapon() != item || PopExtUtil.HasEffect(EF_NODRAW)) return
		...
		function CannotUpgrade(player, item, value = null) {
			...
			item.GetScriptScope().ItemThinkTable[format("CannotUpgrade_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function AlwaysCrit(player, item, value = null) {
            ...
            item.GetScriptScope().ItemThinkTable[format("AlwaysCrit_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function AddCondWhenActive(player, item, value) {
            ...
            item.GetScriptScope().ItemThinkTable[format("AddCondWhenActive_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function DontCountDamageTowardsCritRate(player, item, value = null) {
            ...
            CustomAttributes.TakeDamageTable[format("DmgNoCritRate_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                if (params.weapon != item) return
		...
		function NoDamageFalloff(player, item, value = null) {
            ...
            CustomAttributes.TakeDamageTable[format("NoDamageFalloff_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function CanHeadshot(player, item, value = null) {
            ...
            CustomAttributes.TakeDamageTable[format("CanHeadshot_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                local victim = params.const_entity
                if (params.weapon != item || !victim.IsPlayer() || GetPropInt(victim, "m_LastHitGroup") != HITGROUP_HEAD) return
		...
		function CannotHeadshot(player, item, value = null) {
            ...
            CustomAttributes.TakeDamageTable[format("CannotHeadshot_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                if (params.weapon != item || params.damage_stats != TF_DMG_CUSTOM_HEADSHOT) return
		...
		function CannotBeHeadshot(player, item, value = null) {
            ...
            CustomAttributes.TakeDamageTable[format("CannotBeHeadshot_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function ProjectileLifetime(player, item, value) {
            ...
            item.GetScriptScope().ItemThinkTable[format("ProjectileLifeTime_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function MultDmgVsGiants(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("MultDmgVsGiants_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                local victim = params.const_entity, attacker = params.attacker

                if (victim.IsPlayer() && victim.IsMiniBoss() && params.weapon == item) params.damage *= value
            }
		...
		function MultDmgVsTanks(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("MultDmgVsTanks_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                local victim = params.const_entity, attacker = params.attacker

                if (victim.GetClassname() == "tank_boss" && params.weapon == item) params.damage *= value
            }
		...
		function SetDamageType(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("SetDamageType_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                if (params.weapon == item) params.damage_type = value
            }
		...
		function SetDamageTypeCustom(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("SetDamageType_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
                ...
                if (params.weapon == item) params.damage_stats = value
            }
		...
		function PassiveReload(player, item, value = null) {
            ...
            scope.ItemThinkTable[format("PassiveReload_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                //weapon ent has been destroyed, reference is still valid but the entity isn't
                if (item && !item.IsValid())
                {
                    delete scope.ItemThinkTable[format("PassiveReload_%d_%d", player.GetScriptScope().userid, item.entindex())]
                    return
                }
		...
		function RocketPenetration(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("RocketPenetration_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function ReloadsFullClipAtOnce(player, item, value = null) {
            ...
            scope.ItemThinkTable[format("ReloadFullClipAtOnce_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function MultProjectileScale(player, item, value) {
            ...
            scope.ItemThinkTable[format("MultProjectileScale_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (!("mult projectile scale" in player.GetScriptScope().attribinfo) || player.GetActiveWeapon() != item) return
		...
		function MultCritDmg(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("MultCritDmg_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function ArrowIgnite(player, item, value = null) {
            ...
            item.GetScriptScope().ItemThinkTable[format("ArrowIgnite_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function NoclipProjectile(player, item, value) {
            ...
            local scope = item.GetScriptScope()
            scope.ItemThinkTable[format("NoclipProjectile_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (!("noclip projectile" in player.GetScriptScope().attribinfo) || player.GetActiveWeapon() != item) return
		...
		function ProjectileGravity(player, item, value) {
            ...
            local scope = item.GetScriptScope()
            scope.ItemThinkTable[format("ProjectileGravity_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
                if (!("projectile gravity" in player.GetScriptScope().attribinfo) || player.GetActiveWeapon() != item) return
		...
		function ImmuneToCond(player, item, value) {
            ...
            item.GetScriptScope().ItemThinkTable[format("ImmuneToCond_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
		...
		function CustomKillIcon(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("CustomKillIcon_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                if (params.weapon != item || player.GetActiveWeapon() != item) return
            ...
            CustomAttributes.TakeDamagePostTable[format("CustomKillIcon_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function SetWarpaintSeed(player, item, value) {
            ...
            if (_intsize_ == 8)
            {
                ...
                item.AddAttribute("custom_paintkit_seed_lo", casti2f(
                    digitstore[strlen - 2] << 16 | digitstore[strlen - 1]
                ), -1)
                item.AddAttribute("custom_paintkit_seed_hi", casti2f(
		...
		function AltFireDisabled(player, item, value = null) {
			...
			scope.ItemThinkTable[format("AltFireDisabled_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {
			...
			CustomAttributes.SpawnHookTable[format("AltFireDisabled_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {
		...
		function CustomProjectileModel(player, item, value) {
            ...
            scope.ItemThinkTable[format("CustomProjectileModel_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function() {

                if (!("custom projectile model" in player.GetScriptScope().attribinfo) || player.GetActiveWeapon() != item) return
		...
		function DmgBonusWhileHalfDead(player, item, value) {
            ...
            CustomAttributes.TakeDamageTable[format("ShahanshahAttributeBelowHP_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

                if (!("dmg bonus while half dead" in player.GetScriptScope().attribinfo) || params.weapon != item || player.GetActiveWeapon() != item) return
		...
		function DmgPenaltyWhileHalfAlive(player, item, value) {
			...
			CustomAttributes.TakeDamageTable[format("ShahanshahAttributeAboveHP_%d_%d", player.GetScriptScope().userid, item.entindex())] <- function(params) {

				if (!("dmg penalty while half alive" in player.GetScriptScope().attribinfo) || params.weapon != item || player.GetActiveWeapon() != item) return
	...
	function AddAttr(player, attr_string, value = 0, item = null) {
		...
		foreach(item, attrs in item_table)
		{
			...
			if (item == null || !(attr in CustomAttributes.Attrs)) continue
