...
PopExtUtil.CommentaryNode	 <- @() FindByName( null, "__popext_hide_fcvar_notify" ) || 
								SpawnEntityFromTable( "point_commentary_node", 		 	 { targetname = "__popext_hide_fcvar_notify", commentaryfile = " ", commentaryfilenohdr = " " } )
...
function PopExtUtil::SetPlayerAttributes( player, attrib, value, item = null, customwep_force = false, showhidden = false ) {
	...
	if ( item_handle && item_handle.IsValid() )
		...
		PopExtUtil.ForEachItem( player, @( item ) items[item] <- [attrib, value], true )
	...
	if ( "PopExtAttributes" in ROOT && customattr_function in PopExtAttributes.Attrs ) {
		...
		foreach( item, attrs in items ) {
			...
			if ( !HasItemInLoadout( player, item ) || !( customattr_function in PopExtAttributes.Attrs ) ) 
				continue
...
function PopExtUtil::PlayerBonemergeModel( player, model ) {
	...
	SetPropInt( bonemerge_model, "m_fEffects", 129 )
...
function PopExtUtil::ShowHudHint( text = "This is a hud hint", player = null, duration = 5.0 ) {
	...
	local hudhint = FindByName( null, "__popext_hudhint" ) || SpawnEntityFromTable( "env_hudhint", { targetname = "__popext_hudhint", spawnflags = !player ? 1 : 0, message = text } )
...
function PopExtUtil::AddAttributeToLoadout( player, attribute, value, duration = -1 ) {
	...
	PopExtUtil.ForEachItem( player, function( item ) {
...
function PopExtUtil::AddThink( ent, func ) {
	...
	if ( !( thinktable_func in scope ) ) {
		...
		// ", ent.entindex(), format( "\"%s\"", thinktable_func ), thinktable_func, thinktable_func, thinktable_func, thinktable_func )
		// compilestring(str)()
	}
	...
	if ( func && func != "" ) {
		...
		if ( endswith( typeof func, "function" ) ) {
			...
			scope[ thinktable_name ][ func ] <- func
			return
		}
...
function PopExtUtil::ForEachItem( player, func, weapons_only = false ) {
	...
	local instance = weapons_only ? CBaseCombatWeapon : CEconEntity
	
	for ( local child = player.FirstMoveChild(); child && child instanceof instance; child = child.NextMovePeer() )
		func( child )
}
...
function PopExtUtil::KVStringToVectorOrQAngle( str, angles = false, startidx = 0 ) {
	...
	local split = split( str, separator, true ).apply( @( v ) ToStrictNum( v, true ) )
...
AddEventHook( "post_inventory_application", "UtilPostInventoryApplication", function( params ) {
	...
	printl( PopExtUtil.BotTable )
	printl( PopExtUtil.HumanTable )
	printl( PopExtUtil.PlayerTable )

	printl( player_table.len() )
	printl( human_table.len() )
	printl( bot_table.len() )
