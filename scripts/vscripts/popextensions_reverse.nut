...
{
	foreach (i, a in ammo) if (a == 0) ammo[i] = 1
...
::GivePackReverse <- function(player, pack)
{
	...
	} else if (startswith(classname, "item_healthkit_"))
	{
		...
		SendGlobalGameEvent("player_healed", { patient = GetPlayerUserID(player), healer = 0, amount = hp > maxhp * multiplier ? maxhp - hp : maxhp * multiplier});
		SendGlobalGameEvent("player_healonhit", { entindex = player.entindex(), weapon_def_index = 65535, amount = hp > maxhp * multiplier ? maxhp - hp : maxhp * multiplier });
...
::DrainAmmo <- function(player)
{
	...
	if (((scope.lastreload != timestamp) || (GetItemIndex(activegun) == ITEMINDEX_DRAGONS_FURY && GetPropFloat(player, "m_Shared.m_flItemChargeMeter") < 0.001)) && ammoslot > 0 && scope.sequencearray.find(activegun.GetSequence()) != null)
	{
		...
		if (activegun.GetMaxClip1() == -1 || activegun.GetClassname() == "tf_weapon_compound_bow" || GetPropBool(activegun, "m_bReloadsSingly"))
		{
			...
			if (player.GetPlayerClass() == TF_CLASS_ENGINEER)
			{
				...
				//short circuit drain
					...
					if (InButton(player, IN_ATTACK2) && metalslot >= 65) SetReserveMetal(player, metalslot - 65);
					else if (!InButton(player, IN_ATTACK2)) SetReserveMetal(player, metalslot - 5);
			}
		}
		else
		{
		...
		if (GetItemIndex(activegun) == ITEMINDEX_DRAGONS_FURY) scope.dfdrained = true;
