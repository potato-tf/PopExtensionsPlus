...
function MissionAttributes::MissionAttr(attr, value = 0) {
	...
	case "PlayersAreRobots":
		function MissionAttributes::PlayersAreRobots(params) {
			...
			} else if ("StepThink" in scope.PlayerThinkTable) delete scope.PlayerThinkTable.StepThink // TODO: Need a better system for this, if we change to a mission without PlayersAreRobots, this wont get called
	...
	case "SpellDropRateCommon":
		...
		function MissionAttributes::SpellDropRateCommon(params) {
			...
			local spell = SpawnEntityFromTable("tf_spell_pickup", {
				targetname = "_commonspell"
				origin     = bot.GetLocalOrigin() TeamNum = 2 tier = 0 "OnPlayerTouch": "!self,Kill,,0,-1"
			})
		}
